...
#define INIT_IDLIST(LIST) do { \
    (LIST).list = NULL; \
    (LIST).ilen = (LIST).clen = 0; } while (0)
#define FREE_IDLIST(LIST) do { \
    vim_free((LIST).list); \
    (LIST).list = NULL; \
    (LIST).ilen = (LIST).clen = 0; } while (0)
#define COPY_IDLIST(DST, SRC) do { \
    (DST).list = copy_id_list((SRC).list); \
    (DST).ilen = (SRC).ilen; \
    (DST).clen = (SRC).clen; } while (0)
#define ISNULL_IDLIST(LIST) \
    (!(LIST).list)
#define SETALL_IDLIST(LIST) do { \
...
/* Just what we need to save from a match */
typedef struct
{
    /* Note: I'm thinking we don't need to save this here, as it's saved in spp */
    /*regprog_T		*regprog;*/
    lpos_T		startpos;
    lpos_T		endpos;
} syn_regmmatch_T;
...
typedef struct syn_pattern
{
    ...
    idlist_T	 sp_cont_list;		/* cont. group IDs, if non-zero */
    idlist_T	 sp_next_list;		/* next group IDs, if non-zero */
    struct sp_syn sp_syn;		/* struct passed to in_id_list() */
    char_u	*sp_pattern;		/* regexp to match, pattern */
    regprog_T	*sp_prog;		/* regexp to match, program */
    lpos_T	 sp_startpos0;          /* cached startpos[0] from regmmatch_T */
    lpos_T	 sp_endpos0;		/* cached endpos[0] from regmmatch_T */
#ifdef FEAT_PROFILE
    syn_time_T	 sp_time;
#endif
} synpat_T;
...
typedef struct syn_cluster_S
{
    ...
    idlist_T	    scl_list;	    /* IDs in this syntax cluster */
} syn_cluster_T;
...
#endif
    idlist_T	si_cont_list;		/* list of contained groups */
    idlist_T	si_next_list;		/* nextgroup IDs after this item ends */
...
typedef struct
{
    ...
    idlist_T	next_list;	/* group IDs for "nextgroup" argument */
} syn_opt_arg_T;
...
static int in_id_list(stateitem_T *item, idlist_T *cont_list, struct sp_syn *ssp, int contained);
...
static int get_id_list(char_u **arg, int keylen, idlist_T *list, int skip);
static void syn_combine_list(idlist_T *clstr1, idlist_T *clstr2, int list_op);
					      ...
					      || !ISNULL_IDLIST(cur_si->si_cont_list))
	{
		...
		if (next_match_idx < 0 || next_match_col < (int)current_col)
		{
			...
			if (	   spp->sp_syncing == syncing
			    ...
			    static int dbg_cache_cnt = 0;
			    NOLOG("sp_line_id=%d current_line_id=%d spp->sp_startcol=%d spp->sp_startpos0.col=%d lc_col=%d\n",
				    sp_line_id, current_line_id, spp->sp_startcol, spp->sp_startpos0.col, lc_col);
			    /* TODO: Check any cached match to see whether we can skip the call to syn_regexec. */
			    /* FIXME: Need to use *old* sp_line_id for this test!!!!! Before assignment above! */
			    if (sp_line_id == current_line_id)
			    {
				/* We've already attempted match on this line. */
				if (spp->sp_startcol == MAXCOL)
				    /* No match on this line. */
				    continue;
				else if (spp->sp_startpos0.col >= lc_col) {
				    BPSLOG("%s: Using #%d cache - %s\n", __func__, ++dbg_cache_cnt,
					    syn_id2name(spp->sp_syn.id));
				    /* We have a cached match and it's still valid.
				     * Populate needed portions of regmatch.
				     * Note: spp->sp_prog can't change since we're skipping syn_regexec(). */
				    regmatch.startpos[0] = spp->sp_startpos0;
				    regmatch.endpos[0] = spp->sp_endpos0;
				    r = TRUE;
				}
			    }
			    /* Skip call to syn_regexec if we have a cached match. */
			    if (!r) {
				/* ADDME! */
				...
				/* TODO: Consider moving assignments to rmm_ic
				 * and regprog (above) down here. (I'm assuming
				 * they're needed only when we call
				 * syn_regexec. */
				...
				/* Cache raw positions, which may obviate need for call to syn_regexec next time. */
				/* TODO: Decide whether we should be caching the adjusted positions instead. */
				spp->sp_startpos0 = regmatch.startpos[0];
				spp->sp_endpos0 = regmatch.endpos[0];
			    }
...
syntax_clear(synblock_T *block)
{
    ...
    /* free the list of contained/notcontained/containedin idxs (used to handle
     * TOP/CONTAINED special keywords and containedin groups) */
    ga_clear(&block->b_syn_contained);
    ga_clear(&block->b_syn_notcontained);
    ga_clear(&block->b_syn_containedin);
...
add_keyword(
    ...
    idlist_T	*cont_in_list, /* containedin for this keyword */
    idlist_T	*next_list, /* nextgroup for this keyword */
    int		conceal_char)
{
    keyentry_T	*kp;
    hashtab_T	*ht;
    ...
    int		idx)
{
    ...
    static void
add_syn_item_to_specials(int idx, syn_opt_arg_T *syn_opt_arg)
{
    garray_T *gap;
    ...
    gap = syn_opt_arg->flags & HL_CONTAINED
	? &curwin->w_s->b_syn_contained
 ...
 * BPS TODO: Rework the comment now that idlist_T ptr is input.
 */
    static void
syn_combine_list(idlist_T *clstr1_idlist, idlist_T *clstr2_idlist, int list_op)
{
     ...
     */
    if (ISNULL_IDLIST(*clstr2_idlist))
	return;
    if (ISNULL_IDLIST(*clstr1_idlist) || list_op == CLUSTER_REPLACE)
    {
    ...
    for (round = 1; round <= 2; round++)
    {
	g1 = clstr1_idlist->list;
	g2 = clstr2_idlist->list;
	/* Keep independent counts for normal and cluster ids */
	/* TODO: There's a lot of infrastructure for this that could be
	 * obviated simply by searching backwards at the end, the way it's done
	 * in get_id_list. */
	icount = ccount = 0;
	...
	if (round == 1)
	{
	    ...
	    clstr = (short *)alloc((unsigned)((icount + ccount + 1) * sizeof(short)));
	    if (clstr == NULL)
		break;
	    clstr[icount + ccount] = 0;
	}
    }
...
get_id_list(
    ...
    idlist_T	*list,		/* where to store the resulting list, if not
    ...
    int		skip)
{
    ...
    *arg = p;
    ...
    if (ISNULL_IDLIST(*list)) {
	list->list = retval;
	/* Sort the list, taking care to leave any special keyword at head. */
	short *ps = retval + (has_special ? 1 : 0);
	size_t n = (size_t)total_count - (has_special ? 1 : 0);
    ...
    int		contained)	/* group id is contained */
{
    ...
    if (cur_si != NULL && !ISNULL_IDLIST(ssp->cont_in_list)
					    && !(cur_si->si_flags & HL_MATCH))
    {
